...
export const generateQuotePDF = (
  ...
  return new Promise<void>((resolve, reject) => {
    img.onload = () => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(quote.id.toString(), 15 + doc.getTextWidth("Numéro devis:   "), contentStartY + 21);
    ...
    img.onerror = () => {
      ...
      // Fallback: continue without the logo
      const doc = new jsPDF();
      const pageWidth = doc.internal.pageSize.getWidth();
      const pageHeight = doc.internal.pageSize.getHeight();
      const rightMargin = 15;
      ...
      // Skip logo and continue with the rest of the PDF generation
      doc.setFontSize(50);
      doc.setTextColor(51);
      doc.text(`Devis`, 15, 30);
      doc.setTextColor(0);

      const contentStartY = 40; // Adjusted for missing logo
